...
calc_mort_percent <- function(data) {
  total_individuals <- aggregate(Mortality ~ hpe + NiCl2.Nom.Conc., data = data, FUN = length)
  total_mortality <- aggregate(Mortality ~ hpe + NiCl2.Nom.Conc., data = data, FUN = sum)
  
  merged_data <- merge(total_individuals, total_mortality, by = c("hpe", "NiCl2.Nom.Conc."), suffixes = c(".total", ".count"))
...
For now will not be using Survivorship in final deliverables but may be useful in future toxicology studies \## Survivorship Function
...
ggplot(mort_exp, aes(x = NiCl2.Nom.Conc., y = percentage_mortality)) + 
  ...
  labs(x = "Nominal Nickel (II) Chloride Concentration (mg/L)",
       y = "Mortality (%)") +
...
ggplot(mort_exp, aes(x = NiCl2.Nom.Conc., y = proportion_mortality)) +  # Use proportion for logistic regression
  geom_point(size = 2) +
  facet_wrap(~ hpe) +
...
# Plot the data with sigmoidal (logistic) curve
p <- ggplot(mort_exp, aes(x = NiCl2.Nom.Conc., y = proportion_mortality)) +  # Use proportion for logistic regression
  geom_point(size = 2) +
  facet_wrap(~ hpe) +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = TRUE, color = "black") +  # Use logistic regression
  scale_y_continuous(labels = scales::percent) +  # Convert y-axis back to percentage format
...
model <- glm(proportion_mortality ~ NiCl2.Nom.Conc., data = mort_exp, family = "binomial")
...
```{r}
setwd('..')
png(filename = "output/survival_24_96_2025.png", width = 1200, height = 700)
...
# 50% survival lines
  ...
  geom_segment(aes(x = x_intercept50, xend = x_intercept50, 
               ...
               linetype = "dashed", color = "red3", linewidth = 1)  +
  # 90% survivial lines
 geom_segment(data = fit_data, aes(x = min(surv_24_96$NiCl2.Nom.Conc.), 
                                   xend = x_intercept90, 
                                   y = 90, yend = 90), 
...
*Probit Model:* The probit model assumes that the dose-response relationship follows a cumulative *normal distribution*. It is often used when the response variable is binary (alive/dead) or when the response follows a sigmoidal shape. Suitable when responses are discrete and there's a natural threshold for response.

*Logit Model:* Similar to the probit model, the logit model is used for binary response data. It assumes that the log odds of the response is linearly related to the dose. Like probit, it's suitable for binary responses but may be preferred for computational reasons or if assumptions align better with the data.

*Linear Model:* The linear model assumes a linear relationship between the dose and the response. It may be appropriate when responses are continuous and show a linear trend across doses. However, it may not capture non-linear relationships well, especially if the response plateaus or changes direction.
...
```{r}
#24 h
LC50_24h <- LC_logit((Mortality / Total) ~ log10(NiCl2.Nom.Conc.), p = c(50),
weights = Total,
data = t_LC50[LC50$NiCl2.Nom.Conc. != 0, ],
subset = c(hpe == "24"))

print(LC50_24h)

# 48 h
LC50_48h <- LC_logit((Mortality / Total) ~ log10(NiCl2.Nom.Conc.), p = c(50),
weights = Total,
data = LC50[LC50$NiCl2.Nom.Conc. != 0, ],
subset = c(hpe == "48"))

print(LC50_48h)

# 72 h
LC50_72h <- LC_logit((Mortality / Total) ~ log10(NiCl2.Nom.Conc.), p = c(50),
weights = Total,
data = LC50[LC50$NiCl2.Nom.Conc. != 0, ],
subset = c(hpe == "72"))
...
#96 h
LC50_96h <- LC_logit((Mortality / Total) ~ log10(NiCl2.Nom.Conc.), p = c(50),
weights = Total,
data = LC50[LC50$NiCl2.Nom.Conc. != 0, ],
subset = c(hpe == "96"))
print(LC50_96h)
...
ggplot(data = t_LC50[t_LC50$NiCl2.Nom.Conc. != 0, ],
  aes(x = NiCl2.Nom.Conc., y = (Mortality / Total))) +
  geom_point() +
  geom_smooth(method = "glm",
   method.args = list(family = binomial(link = "logit")),
 aes(weight = Total), colour = "#FF0000", se = TRUE)
...
```{r}
glmm_model <- glmer(Mortality ~ hpe + NiCl2.Nom.Conc. + (1|Tunicate.ID),
                    data = t_LC50,
...
```{r}
t_LC50_high_dose <- subset(t_LC50, NiCl2.Nom.Conc. == 1000)
...
r_squared_values_mean <- mean_health_score %>%
  ...
  do({
    ...
    data.frame(Ni.Conc = unique(.$NiCl2.Nom.Conc.), r_squared = r_squared)
  }) %>%
  ungroup()
...
ggplot() + 
  ...
  facet_wrap(~ NiCl2.Nom.Conc., labeller = as_labeller(c("0" = "0 mg/L", "0.1" = "0.1 mg/L", "1" = "1 mg/L", "10" = "10 mg/L", "100" = "100 mg/L", "1000" = "1000 mg/L"))) +
  scale_x_continuous(breaks = c(0, 24, 48, 72, 96), expand = c(0.05, 0.1)) +
  scale_y_continuous(breaks = seq(0, 10, by = 2), limits = c(0,10)) +
  geom_smooth(data = t_LC50_clean, aes(x = hpe, y = Health), method = "lm", se = TRUE, fill= "lightsteelblue2", color = "lightsteelblue3", linewidth = 4, formula = (y ~ x), span = 1) +  # Add linear regression line with SE based on raw data
  labs(x = "Time (hours)", 
       y = "Health Score"
       ) +
  theme_test() +
...
```{r}
setwd('..')
png(filename = "output/health_mean_se_combined.png", width = 2500, height = 1300)

ggplot() + 
  
  # Mean points with SE bars, colored by treatment
  geom_line(data = mean_health_score, 
            aes(x = hpe, y = mean_health, color = as.factor(NiCl2.Nom.Conc.)), 
            linewidth = 4, alpha = 0.9) +
  geom_point(data = mean_health_score, 
             aes(x = hpe, y = mean_health, color = as.factor(NiCl2.Nom.Conc.)), 
             size = 8) +
  geom_errorbar(data = mean_health_score, 
                aes(x = hpe, ymin = mean_health - se, ymax = mean_health + se, 
                    color = as.factor(NiCl2.Nom.Conc.)), width = 4, linewidth = 1.5) +
  ...
  scale_color_viridis_d(
    option ="C",
    name = expression(paste("Nom. NiCl"[2]," (mg/L)")),
  ...
  theme(
    ...
    legend.box.margin = margin(t = 0, r = 1, b = 0, l = 1, "cm"),
    axis.title = element_text(size = 60),
    axis.text.y = element_text(size = 55, hjust = 1, margin = margin(r = 20)),
    axis.text.x = element_text(size = 55, vjust = 4.5, hjust = 0.5, margin = margin(t = 50)),
...
ggplot() + 
  ...
  geom_point(data = mean_health_score, aes(x = hpe, y = mean_health, color = as.factor(NiCl2.Nom.Conc.)), size = 4) +  # Mean points with color by NiCl2.Nom.Conc.
  geom_errorbar(data = mean_health_score, aes(x = hpe, ymin = mean_health - se, ymax = mean_health + se, color = as.factor(NiCl2.Nom.Conc.)), width = 3) +  # Error bars with color by NiCl2.Nom.Conc.
  geom_smooth(data = t_LC50_clean, aes(x = hpe, y = Health, color = as.factor(NiCl2.Nom.Conc.), fill = as.factor(NiCl2.Nom.Conc.)), method = "lm", se = TRUE, linewidth = 2) +  # Add linear regression line with SE based on raw data
...
ggplot() + 
  geom_point(data = t2_LC50, aes(x = NiCl2.Nom.Conc.trans, y = Health), size = 0.01, color = "white", alpha = 0.5) +  # Raw data points for SE calculation
  geom_point(data = mean_health_score_no0hpe, aes(x = NiCl2.Nom.Conc.trans, y = mean_health), size = 4, color = "black") +  # Mean points
  geom_errorbar(data = mean_health_score_no0hpe, aes(x = NiCl2.Nom.Conc.trans, ymin = mean_health - se, ymax = mean_health + se), width = 0.2) +  # Error bars
  facet_wrap(~ hpe, labeller = as_labeller(c("24" = "24 hpe", "48" = "48 hpe", "72" = "72 hpe", "96" = "96 hpe"))) +
  ...
  geom_smooth(data = t2_LC50, aes(x = NiCl2.Nom.Conc.trans, y = Health), method = "lm", se = TRUE, color = "darkblue", linewidth = 5) +  # Add linear regression line with SE based on raw data
  labs(x = "Nickel Concentration (mg/L)", 
       y = "Health Score"
       ) +
  theme_test() +
...
NOT IMPORTANT TO DO DO NOT PROCEED. For now this information regarding decline in health will remain descriptive using the above graphs as it distracts from the purpose of the study. 
```{r}
...
model_summaries <- lm_models %>% 
  summarize(
    NiCl2.Nom.Conc. = unique(NiCl2.Nom.Conc.),
...
combined_model <- lm(mean_health ~ hpe * NiCl2.Nom.Conc., data = mean_health_score)
...
leveneTest(Health ~ NiCl2.Nom.Conc.*hpe, data = fact_LC50)
...
leveneTest(Health_sqrt ~ NiCl2.Nom.Conc., data = t_LC50)

# Run ANOVA on transformed data
#anova_model <- aov(Health_sqrt ~ NiCl2.Nom.Conc., data = t_LC50)
#summary(anova_model)
...
This bit needs to be fixed and also further researched. As above there is evidently a signficant difference in the slopes by NiCl2.Nom.Conc.:hpe but at the moment can't say exactly which are different from which.

```{r}
library(emmeans)
# Simple slopes analysis
simple_slopes <- emmeans(combined_model, ~ hpe | NiCl2.Nom.Conc.)
