...
```{r}
metadata <- metadata %>%
  select(hom_num, Tunicate.ID, date, hpe, NiCl2.Nom.Conc., true_rep, rep_exp) %>%
  mutate(NiCl2.Nom.Conc. = as.factor(NiCl2.Nom.Conc.)) %>%
  mutate(hpe = as.factor(hpe)) %>%
  mutate(hom_num = as.factor(hom_num))
summary(metadata)
...
combo <- inner_join(combo, blast, by = c("Tunicate.ID", "NiCl2.Nom.Conc.", "hpe"))
summary(combo)
```
...
```{r, eval=TRUE}
ggplot(combo, aes(x = NiCl2.Nom.Conc., y = sod_avg, fill = NiCl2.Nom.Conc.)) +
  geom_point() +
  geom_violin(trim = FALSE) +
...
ggplot(combo_plot, aes(x = NiCl2.Nom.Conc., y = sod_u_mg)) +
  ...
  scale_y_continuous(breaks = seq(0, 40, by = 10), limits = c(0, 40), expand = c(0.2, 0)) +
...
```{r}
png(filename = "../output/boxplot_sod.png", width = 1300, height = 850)
ggplot(combo_plot, aes(x = NiCl2.Nom.Conc., y = sod_u_mg)) +
  geom_boxplot(width = 0.7, outlier.shape = NA, alpha = 0.6) +
  geom_jitter(width = 0.12, height = 0, size = 4, alpha = 0.9) +
  stat_summary(fun = median, geom = "point", size = 4, color="blue") +
  facet_wrap(~ hpe, nrow = 1,
             labeller = labeller(
    hpe = function(x) paste0(x, " hpe"))) +
  scale_y_continuous(breaks = seq(0, 40, by = 10), limits = c(0, 40), expand = c(0.05, 0.1)) +
  labs(x = "Nickel (II) Chloride Concentration (mg/L)", y = "Median SOD1 Activity (U/mg protein)") +
  geom_text(
  data = n_counts,
  aes(x = NiCl2.Nom.Conc., y = 0, label = paste0("n=", n)),
  vjust = 1.5,
