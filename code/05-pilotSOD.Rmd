...
```{r}
png(filename = "../output/SOD_standard_curve.png", width = 900, height = 750)
ggplot(std_df, aes(x = known_sod_u_ml, y = blank_corrected_abs)) +
  facet_wrap(~plate, 
             labeller = labeller(plate = function(x) paste("Plate", x))) +
  geom_point(size = 4) +
  geom_line(
    data = sod_lines,
  ...
  theme_classic(base_size = 22) +  # larger overall text
  theme(
...
bca <- bca %>%
  mutate(
    ...
    protein_mg_ml_final = if_else(
      type == "unknown",
      protein_mg_ml_calc * 5,   # 4 sample: 16 PBS (uL) so actually a 1:5 dilution. ignore columns and multiply by 5
...
png(filename = "../output/BCA_standard_curve.png", width = 900, height = 750)
...
ggplot(bca_std_df, aes(x = known_mg_ml, y = abs562_blankcorr)) +
  ...
  geom_text(
    data = bca_fit_stats,
    aes(label = paste0("R² = ", sprintf("%.3f", r_squared))),
    x = Inf,
    y = Inf,
    hjust = 1.1,
    vjust = 1.5,
...
```{r}
# coefficients per plate (with intercept) compare for sanity check that we should force the model through (0,0)
...
#extract homogenate, tech rep, and dilution factor from the well_content column
pierce <- pierce %>%
  mutate(
...
pierce <- pierce %>%
  mutate(
    ...
    protein_mg_ml_final = if_else(
      ...
      protein_mg_ml_calc * 5, # did a dilution of 4 ul of protein to 16 ul of pbs (so even though it says 1:4 its a dilution of 5)
      protein_mg_ml_calc
    )
  )
...
# pierce data appears too variable and inconsistent. For this analysis, we will keep to BCA assay for normalziation in final figures but exploratory plots may be made below with pierce assay normalized sod

```

calculate sod per mg of protein for each assay
...
Since all graphs below will use the bca data to normalize (had a more robust standard curve) let us filter out just na for sod_u_mg_bca. 
```{r}
combo <- combo %>%
  filter(!is.na(sod_u_mg_bca)) %>%
...
stage by 24 hours post exposure
```{r}

ggplot(combo24, aes(x = NiCl2.Nom.Conc., y = sod_u_mg_bca, fill = hpe)) +
  geom_point() +
...
```
stage by 96 hours post exposure
```{r}

ggplot(combo96, aes(x = NiCl2.Nom.Conc., y = sod_u_mg_bca, fill = hpe)) +
  geom_point() +
  geom_violin(trim = FALSE) +
  facet_wrap(~ simple_stage) +
...
ggplot(combo, aes(x = NiCl2.Nom.Conc., y = sod_u_mg_bca, fill = hpe)) +
  geom_point() +
  geom_violin(trim = FALSE) +
...
ggplot(combo_plot, aes(x = NiCl2.Nom.Conc., y = sod_u_mg_bca)) +
...
```{r}
png(filename = "../output/presentation_boxplot_sod_RnormalizedBCA_alt.png", width = 1000, height = 850)
ggplot(combo_plot, aes(x = NiCl2.Nom.Conc., y = sod_u_mg_bca)) +
...
```{r}
library(car)
# make sure independent variables are saved as factors in data frame
combo <- combo %>%
  mutate(
    hpe = factor(hpe),
    NiCl2.Nom.Conc. = factor(NiCl2.Nom.Conc.)
...
fit <- lm(sod_u_mg_bca ~ hpe * NiCl2.Nom.Conc., data = combo)

# ANOVA table (base)
anova(fit)

# Better for unbalanced designs than aov() function in base R
Anova(fit, type = 2)   
...
hist(combo$sod_u_mg_bca, main = "Distribution", xlab = "SOD U/mg")
```
...
emm <- emmeans(fit, ~ NiCl2.Nom.Conc.) #since only ni treatment produced a sig p value, we will collapse across hpe (time) and evaluate pairwise comparisons for just treatment
pairs(emm, adjust = "tukey")

#notes: maybe run an LSD
...
Post-hoc Tukey–Kramer comparisons conducted collapsing timepoints and evaluating ni conc only saw no differences among concentrations p > 0.05. At 96 hpe, SOD activity at 1 mg/L was elevated relative to controls, though this difference did not reach statistical significance after multiple-comparison correction (p = 0.054). No differences were detected between 100 mg/L and other concentrations.
...
## Krusk Wallis test non parametric analysis

```{r}

kruskal.test(sod_u_mg_bca ~ NiCl2.Nom.Conc., data = combo) # all data collapsed across time points
kruskal.test(sod_u_mg_bca ~ NiCl2.Nom.Conc., data = combo96) # for 96 hpe
kruskal.test(sod_u_mg_bca ~ NiCl2.Nom.Conc., data = combo24) # for 24 hpe
```

```{r}
library(rstatix)
#just the post hoc for 96 hpe
dunn_test(sod_u_mg_bca ~ NiCl2.Nom.Conc., p.adjust.method = "holm", data= combo96)
```


so for 96 hpe there is a significant difference 
